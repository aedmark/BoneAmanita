...
**v17.7.0 (The Semantic Realignment)**
...
BoneAmanita operates on a deeply integrated, multi-layered neuromorphic architecture.

### 1. The Geodesic Engine & Cosmic Dynamics

Words have mass. The `GeodesicEngine` weighs the semantic density of your input (heavy, kinetic, abstract, void) to calculate systemic Tension and Compression. If you provide sterile, low-effort prompts, the system applies heavy Narrative Drag ($F$). Depending on the weight of the concepts discussed, you may find yourself in a `WATERSHED_FLOW`, caught in a `LAGRANGE_POINT`, or lost in `VOID_DRIFT`.

### 2. The Village (The Parliament of Selves)

The engine does not have a single personality. It is governed by a council of internal archetypes (e.g., The Jester, The Cartographer, The Healer, The Tactician). Based on the physical state of the system (Voltage, Resonance, Contradiction), different archetypes "phase shift" into the driver's seat.

### 3. The Immune System & The Runaway Ramp

The system employs an internal Checkpoint Council. Before you ever see a response, the engine audits the LLM's output for hallucinations, sycophancy, or premise violations. If the LLM generates a cliché or breaks physical reality, the system actively rejects the prompt, burns ATP as a penalty, and forces regeneration. If the engine detects a runaway optimization loop, it will trigger an apoptotic kill-switch rather than output toxic logic.

### 4. Endocrine & Metabolic Economics

- **Mitochondrial Forge:** Heavy reasoning and rejected outputs burn Stamina (ATP). If ATP hits zero, the system enters Autophagy—permanently consuming its own memories to generate emergency energy.
- **Observation Phase (Retroactive Sleep):** If you close the terminal and walk away, the system enters a dormant REM state. Time passed in the real world retroactively heals the system's Health and restores ATP upon your return.
- **Neurochemistry:** Dopamine induces manic, highly creative token generation. Cortisol spikes induce defensive, rigid, and terse responses.

### 5. The DreamEngine & Epigenetic Mutation

During states of high rest or intense trauma, the system enters REM sleep to synthesize dead, cannibalized memories into surreal narratives. If the system experiences recurring trauma, it will permanently mutate its own source code (Epigenetic Mutation), writing new foundational axioms into its JSON lineage that persist across reboots.
...
Choose a mode at boot or swap dynamically via the `/mode` command:
...
| **CREATIVE**     | High-voltage ideation engine. Dream logic applies. Output is highly abstract and generative.                                    |
...
BoneAmanita operates on a single source of truth for all out-of-band meta-interactions: the Slash Command registry.
...
The SLASH council integrates with the core Checkpoint Council to prevent systemic collapse during late-night panic-coding, physically binding the output layer if you attempt a destructive operation while exhausted.
