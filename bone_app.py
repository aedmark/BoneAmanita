...
def format_log_entry(log_str):
    ...
    if "TOWN HALL" in clean: return f"📜 {clean.replace('📜', '').strip()}"
    if "VITAL SIGNS" in clean: return f"🩺 {clean.replace('🩺', '').strip()}"
    if "PARADOX BLOOM" in clean: return f"🌷 {clean.replace('🌷', '').strip()}"
    if "CARTOGRAPHER" in clean: return f"🗺️ {clean.replace('🗺️', '').strip()}"
