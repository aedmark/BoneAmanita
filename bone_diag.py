...
class TrueEngineTest(BoneTestCase):
 ...
 "panic_clean_words": ["safe", "warm", "blanket"],}
    ...
    def test_autophagy_memory_cannibalization(self):
        memory_graph = (self.engine.mind.mem.graphif hasattr(self.engine.mind, "mem")else self.engine.akashic.graph)
    ...
    def test_retroactive_metabolism_and_sleep_isolated(self):
        ...
        try:
            ...
            ctx.limits = (getattr(self.engine.bone_config, "CYCLE", {}).__dict__if hasattr(getattr(self.engine.bone_config, "CYCLE", None), "__dict__")else {})
            ctx = phase.run(ctx)
    ...
    def test_reconstructive_memory_drift(self):
        ...
        mem_core.graph["ECHO_NODE"] = {
            "edges": {"original_context": 10.0, "core_strut": 10.0},
            "last_tick": 0,
        }
        ...
        self.assertEqual(
            edges.get("original_context"),
            9.5,
            "Memory failed to decay its original edges by 5% during recall.",
        )
        self.assertEqual(
        ...
        self.assertTrue(
            any(k in mem_core.dimension_map["PSI"] for k in new_keys),
            "Injected context did not match the active PSI dimension.",
        )
    ...
    def test_apoptotic_kill_switch_cause(self):
        ...
        cause = DeathGen._determine_cause(
            phys, {"atp": 50.0}, config_ref=self.engine.bone_config
        )
        self.assertEqual(
            cause,
            "APOPTOSIS",
    ...
    def test_rejection_death_loop_mercy_rule(self):
        ...
        self.assertEqual(
            drag_val, 999.0, "Mercy Rule failed to spike narrative drag to infinity."
        )
        self.assertIn(
...
class FractureEngineTest(BoneTestCase):
    ...
    def test_fracture_live_toxicity(self):
        ...
        self.assertGreater(
            ros_toxicity,
            0.0,
            "The system failed to accumulate ROS toxicity from the LLM slop.",
        )
        self.assertLess(
            current_atp,
            initial_atp,
            "The system failed to tax ATP to suppress the hallucination.",
        )
        self.assertIn(
            "MERCY RULE",
            str(result) + str(self.engine.events.flush()),
            "The ResponseValidator failed to flag the output as toxic/slop.",
        )
    ...
    def test_fracture_novelty_spade(self):
        ...
        self.assertEqual(
            physics_state.get("ros"), 10.0, "[FAIL] Cortisol (ROS) did not drop by 10."
        )
        self.assertEqual(
    ...
    def test_fracture_ralph_apoptosis(self):
        ...
        if (
            not hasattr(self.engine.cortex, "last_physics")
    ...
    def test_fracture_runaway_ramp(self):
        ...
        self.assertIn(
            "RHODES", response, "[FAIL] Rhodes did not apply absolute friction."
        )
        self.assertEqual(
    ...
    def test_fracture_systemic_health_medical_chip(self):
        ...
        hit, logs, corr, man = overseer.audit(
            "[MOD:SYSTEMIC_HEALTH] fix this", physics_decay
        )
        ...
        self.assertEqual(
            corr.get("h_s"), -0.1, "[FAIL] Holistic resilience was not taxed."
        )
        self.assertTrue(
        ...
        hit, logs, corr, man = overseer.audit(
            "[MD] just force it to compile", physics_broken
        )
        self.assertTrue(
        ...
        self.assertEqual(
            corr.get("ros"), -100.0, "[FAIL] Linehan did not force ROS to 0."
        )
        self.assertEqual(
            corr.get("r_a"), 1.0, "[FAIL] Radical Acceptance Index not maximized."
        )
        self.assertTrue(
    ...
    def test_the_fracture(self):
        ...
        start_time = time.time()
        results = network.retrieve_semantic(
            "trigger_word", query_vector, scope=0.9, resonance=0.5
        )
        ...
        self.assertEqual(
            nodes_moved, 10000, "Consolidator failed to move all 10,000 nodes."
        )
    ...
    def test_fracture_aerodynamic_lift(self):
        ...
        print(
            f"  [SUCCESS] Engine achieved aerodynamic lift: {forces['compression']} Drag."
        )
    ...
    def test_fracture_gatekeeper_metrics_padding(self):
        ...
        self.assertIn(
            "type", result, "[FAIL] Gatekeeper did not return a valid packet type."
        )
        self.assertIn(
            "SYNTAX_ERR",
            result["type"],
            "[FAIL] Gatekeeper did not flag the payload as a syntax error.",
        )
        self.assertIn(
            "metrics",
            result,
