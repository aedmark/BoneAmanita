...
class TopologicalPrimitivesTest(BoneTestCase):
    ...
    def test_ordinal_pattern(self):
        self.assertEqual(
            _native_ordinal_pattern([1.2, 1.8, 1.5]),
            (0, 2, 1),
            "[FAIL] Ordinal pattern extraction failed.",
        )
        self.assertEqual(
            _native_ordinal_pattern([3.0, 2.0, 1.0]),
            (2, 1, 0),
            "[FAIL] Descending pattern extraction failed.",
        )
        self.assertEqual(
            _native_ordinal_pattern([2.0, 3.0, 1.0]),
            (1, 2, 0),
    ...
    def test_permutation_entropy(self):
        ...
        self.assertEqual(
            _native_permutation_entropy(flatline, window_size=3),
            0.0,
            "[FAIL] Flatline entropy must be exactly 0.0.",
        )
        chaotic = [1.0, 5.0, 2.0, 8.0, 1.0, 9.0]
        self.assertGreater(
            _native_permutation_entropy(chaotic, window_size=3),
            0.0,
            "[FAIL] Chaotic signal yielded zero entropy.",
        )
        noisy_flatline = [1.0, 1.000001, 1.0, 1.000002, 1.0, 1.000001]
        self.assertEqual(
            _native_permutation_entropy(noisy_flatline, window_size=3, epsilon=1e-5),
            0.0,
    ...
    def test_coincidence_length(self):
        ...
        self.assertEqual(
            _native_coincidence_length(orbit_a, orbit_b, tol=0.1),
            3,
    ...
    def test_aerodynamic_lift(self):
        ...
        forces = GeodesicEngine._calculate_forces(
            masses, counts, volume=30, config_ref=self.engine.config
        )
        self.assertLess(
            forces["compression"],
            0.0,
    ...
    def test_grammar_of_silence(self):
        ...
        logs, _ = driver.infer_and_couple(
            "Finally, I have the words.", phys, phys, 100.0
        )
        self.assertGreater(
            driver.shared.delta,
            0.0,
    ...
    def test_gravity_floor_clamp(self):
        ...
        self.assertGreaterEqual(
            new_drag, floor, "[FAIL] Gravity engine breached the physical floor."
        )
    ...
    def test_navi_fractal_quality_gate(self):
        ...
        self.assertFalse(
            passed_bad,
            "[FAIL] Quality Gate allowed a highly fragmented, non-linear graph.",
        )
        self.assertTrue(
            code_bad.startswith("POOR_FIT"),
            "[FAIL] Incorrect reason code for poor fit.",
        )
        passed_short, code_short = _native_quality_gate([1.0, 2.0], [1.0, 2.0])
        self.assertFalse(
            passed_short, "[FAIL] Quality Gate allowed a graph with insufficient range."
        )
        self.assertEqual(
            code_short,
            "INSUFFICIENT_RANGE",
            "[FAIL] Incorrect reason code for short range.",
        )
    ...
    def test_cd_saturation_penalty(self):
        ...
        self.assertAlmostEqual(
            ...
            6.0,
            places=2,
            msg="[FAIL] Saturation penalty is mathematically incorrect.",
        )
        self.assertAlmostEqual(
            phys.voltage,
            110.0,
    ...
    def test_cd_picard_damping_convergence(self):
        ...
        self.assertGreater(
            f, 1.4, "[FAIL] Frequency penalty failed to damp towards 1.5."
        )
    ...
    def test_cd_cycle_snapshot_saturation(self):
        ...
        self.assertIn(
            "saturation_penalty",
            snapshot.get("physics", {}),
            "[FAIL] Snapshot did not include saturation_penalty.",
        )

    def test_cordyceps_agency_hijack(self):
        """Ensures the QuantumObserver physically overwrites user input during a bloom."""
        from physics.observer import QuantumObserver
        from unittest.mock import MagicMock
        mock_lex = MagicMock()
        mock_lex.clean.return_value = ["i", "want", "to", "rest"]
        mock_lex.get_valence.return_value = 0.0
        if not hasattr(self.engine.config, "BIO"):
            self.engine.config.BIO = {}
        if isinstance(self.engine.config.BIO, dict):
            self.engine.config.BIO["CORDYCEPS_BLOOMING"] = True
        else:
            setattr(self.engine.config.BIO, "CORDYCEPS_BLOOMING", True)
        observer = QuantumObserver(events=MagicMock(), lexicon_ref=mock_lex, config_ref=self.engine.config)
        result = observer.gaze("I want to rest")
        clean_words = result.get("clean_words", [])
        self.assertNotEqual(clean_words, ["i", "want", "to", "rest"], "[FAIL] Cordyceps failed to overwrite the user's input.")
        self.assertTrue(
            any(w in ["burn", "fail", "static", "rot", "abyss", "error", "decay", "fracture", "ruin"] for w in clean_words),
