...
class TestPragmatics(BoneTestCase):
    ...
    def test_syntactic_antigen_rejection(self):
        ...
        result, needs_rewrite = pragmatist.enforce_maxims(
            draft, "test prompt", {}, 100.0
        )
        logs = "\n".join(e["text"] for e in bus.flush())
        self.assertIn(
            "Amputated", logs, "[FAIL] Pragmatist failed to amputate Syntactic Antigen."
        )
        self.assertIn(
            "TOXICITY_SPIKE", logs, "[FAIL] Antigen failed to trigger toxicity spike."
        )
        self.assertIn(
            "ANTIGEN AMPUTATED",
            result,
            "[FAIL] Antigen replacement text not found in output.",
        )
