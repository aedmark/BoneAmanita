...
from soul.narrativeself import CoreMemory, NarrativeSelf
...
class SoulSubstrateTests(BoneTestCase):
    def test_trait_vector_dynamic_normalization(self):
        ...
        self.assertAlmostEqual(
            tv.curiosity, 0.75, msg="Curiosity failed to normalize dynamically."
        )
        self.assertAlmostEqual(
            tv.cynicism, 0.25, msg="Cynicism failed to normalize dynamically."
        )
        self.assertAlmostEqual(
            tv.wisdom,
            0.3,
            msg="Wisdom failed to respect its unique dynamic target of 0.1.",
        )
    ...
    def test_narrative_self_ux_safety(self, mock_ux, mock_ux_format):
        ...
        soul = NarrativeSelf(
            engine_ref=MagicMock(), events_ref=events_mock, memory_ref=memory_mock
        )
        try:
    ...
    def test_oroboros_missing_lore_safety(self, mock_ux, mock_ux_format):
        ...
        soul_mock.core_memories = [
            CoreMemory(
                timestamp=0,
                trigger_words=["void"],
                emotional_flavor="LUCID",
        ...
        try:
            ...
            self.fail(
                f"[CRITICAL] Oroboros apply_legacy crashed with missing UX strings: {e}"
            )
