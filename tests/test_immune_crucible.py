...
class TestImmuneCrucible(BoneTestCase):
    ...
    def test_semantic_prion_apoptosis(self):
        ...
        self.assertTrue("semantic prion" in str(snapshot.get("logs", [])), "Gatekeeper failed to log the specific immune response.")
    ...
    def test_zen_flush_trauma_clear(self):
        ...
        self.assertEqual(self.engine.stamina, getattr(self.engine.config, "MAX_STAMINA", 100.0), "Zen failed to restore stamina.")
    ...
    def test_moog_apoptotic_cascade(self, mock_death):
        mock_death.return_value = {"type": "DEATH", "ui": "MOOG: Apoptotic Gate triggered.", "logs": ["Runaway loop exceeds Immune Competence."]}
        self.engine.observer.last_physics_packet = {"entropy": 2.0, "i_c": 0.1, "narrative_drag": 999.0}
        ...
        self.assertEqual(snapshot.get("type"), "DEATH", "Engine failed to apoptose under fatal mathematical malignancy.")
        mock_death.assert_called_once()
    ...
    def test_dual_path_divergence_lock(self):
        ...
        self.assertIn("Dual-Path divergence detected", str(snapshot.get("logs", [])), "Engine failed to identify the structural fracture.")
